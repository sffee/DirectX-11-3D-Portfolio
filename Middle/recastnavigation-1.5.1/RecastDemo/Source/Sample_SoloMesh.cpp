...
bool Sample_SoloMesh::handleBuild()
{
	...
	if (m_cfg.maxVertsPerPoly <= DT_VERTS_PER_POLYGON)
	{
		...
		if (!dtCreateNavMeshData(&params, &navData, &navDataSize, ImportData))
		{
	...
	fopen_s(&SaveFile, "Test.obj", "wt");

	if (nullptr != SaveFile)
	{

		for (size_t i = 0; i < ImportData.size(); i += 3)
		{
			std::stringstream Stream;

			Stream << "v " << ImportData[i + 0] << " " << ImportData[i + 1] << " " << ImportData[i + 2] << "\n";
			fwrite(Stream.str().c_str(), Stream.str().size(), 1, SaveFile);
		}
		...
		for (int i = 0; i < m_navMesh->getMaxTiles(); ++i)
		{
			...
			for (int j = 0; j < tile->header->polyCount; ++j)
			{
				...
				Stream << "f " << p->verts[0] + 1 << " " << p->verts[1] + 1 << " " << p->verts[2] + 1 << "\n";
				fwrite(Stream.str().c_str(), Stream.str().size(), 1, SaveFile);
			}
		}
